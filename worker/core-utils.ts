...
export const bad = (c: Context, error: string, issues?: any) => c.json({ success: false, error, issues } as ApiResponse, 400);
export const notFound = (c: Context, error = 'not found') => c.json({ success: false, error } as ApiResponse, 404);
export const serverError = (c: Context, error = 'Internal Server Error') => c.json({ success: false, error } as ApiResponse, 500);
